...
<div class="accordion accordion-flush" id="itemLista">
  <div class="accordion-item">
    <h2 class="accordion-header" id="flush-heading${heading}">
    ...
    <div id="flush-collapse${heading}" class="accordion-collapse collapse" aria-labelledby="flush-heading${heading}" data-bs-parent="#accordionFlushExample">
      ...
      document.getElementById("contenedor").innerHTML = `<h2>Servicios:</h2>` + html + `<button class="btn btn-dark" id="atrasbtn" type="button">Atrás</button>`;
...
function crear() {
  document.getElementById("contenedor").innerHTML = `
    <h2>Cargá la info de tu evento:</h2>
    <form id="crearEvento">
    <div class="mb-3">
  ...
  function pushData(e) {
    ...
    validar ? swal(`El registro de tu ${consulta.nombre} te costaria aproximadamente $${consulta.precio}ARS`) : swal(`${consulta.nombre} no es válido como evento, por favor revisa tus datos.`);
  }
  document.getElementById("atrasbtn").onclick = function () {
    atras();
  };
...
const btnBorrar = document.getElementById("borrarHist").addEventListener("click", borrar);
...
function atras() {
  document.getElementById("contenedor").innerHTML = `
        <h2>Consulta precios y horarios acá ;)</h2>
...
// // //spread
// let eventosPredeterminados = [...listaServicios];
// console.log(`evento predeterminado:`, eventosPredeterminados);

// // //desesctructuracion
// let sinDisponibilidad = ({ nombre, duracion }) => {
//   fetch("/listado.json")
//     .then((res) => res.json())
//     .then((listaServicios) => {
//       console.log(`ATENCIÓN: Los ${nombre}s no podrán exceder las ${duracion} por falta de disponibilidad, Gracias.`);
//     });
// };
// sinDisponibilidad(listaServicios);
